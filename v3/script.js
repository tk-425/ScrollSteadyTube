function injectStyle(container) {
  ...
  setTimeout(() => {
    ...
    try {
      ...
      playerWideContainer = box[0].childNodes[16];

      // default-player element
      player = box[0].childNodes[20].childNodes[1].childNodes[1].childNodes[1];

      // background element of default-player
      cinematicContainer = player.childNodes[1];

      // full-screen element
      fullscreenContainer = box[0].childNodes[16].childNodes[1].childNodes[4];
